...
const LandingPage: React.FC = () => {
  ...
  const userLabel = useMemo(() => {
    return currentUser?.displayName?.trim() || currentUser?.email?.trim() || 'User';
  }, [currentUser]);

  const userInitial = useMemo(() => {
    return userLabel.charAt(0).toUpperCase();
  }, [userLabel]);

  function openCard(destination: string) {
    // Before sign-in, send card clicks to the auth choice screen.
    // Once signed in during the current app session, open pages directly.
    if (loading) return;
    ...
    navigate('/auth', {
      state: {
        ...
        message: 'Please sign in or create an account to continue to this feature.',
      },
    });
  }
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-brand-text-primary">
      ...
      <nav className="border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-2xl font-bold tracking-tight">OncoScanAI</span>
          </div>
            ...
            {currentUser ? (
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className="h-11 w-11 rounded-full bg-brand-pink text-white font-bold text-sm shadow-sm hover:opacity-90 transition-all flex items-center justify-center"
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-3 w-52 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                    ...
                    <button
                      ...
                      className="mt-2 w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"
                    >
                      Logout
                    </button>
                  </div>
              ...
              <>
                <Link to="/auth" className="px-5 py-2.5 bg-brand-text-primary text-white rounded-full hover:bg-black transition-all shadow-sm">
                  Login
                </Link>
                <Link to="/auth" className="px-5 py-2.5 bg-brand-pink text-white rounded-full hover:opacity-90 transition-all shadow-sm">
                  Sign Up
                </Link>
              </>
            )}
            ...
            <div className="mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl">
              {[
                ...
                <button
                  ...
                  type="button"
                  disabled={loading}
                  onClick={() => openCard(card.to)}
                  className="group relative text-left w-full bg-white border border-slate-200 border-t-4 border-t-brand-pink rounded-2xl p-5 shadow-sm transition-all hover:shadow-lg hover:-translate-y-1 hover:border-brand-pink/60 hover:bg-gradient-to-br hover:from-pink-50 hover:via-fuchsia-50 hover:to-purple-50 active:translate-y-0 active:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-pink/40 cursor-pointer disabled:opacity-60 disabled:pointer-events-none disabled:hover:translate-y-0 disabled:hover:shadow-sm"
                >
                  {/* Lock / verifying badge */}
                  {(loading || !currentUser) && (
                    <span className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-semibold">
                      {loading ? (
                        <>
                  ...
                  <span className="text-xs font-bold text-brand-pink">
                    {loading ? 'One moment…' : currentUser ? 'Get Started →' : 'Sign in or sign up →'}
                  </span>
                </button>
              ))}
            </div>
      ...
      <footer className="py-12 border-t border-slate-100 text-center text-sm text-slate-500">
        <div className="container mx-auto px-6">
          <p>© {new Date().getFullYear()} OncoScanAI. For professional clinical use only.</p>
        </div>
      </footer>
    </div>
  );
